...
namespace nn.friends
{
    public static partial class Friends
    {
        ...
        public static nn.Result StartFriendInvitation(nn.account.Uid uid, int maxInviteeCount,
            nn.friends.FriendInvitationGameModeDescriptionV2 description,
            byte[] pAppParameter)
        {
            return new nn.Result();
        }
        ...
        public static nn.Result StartFriendInvitation(nn.account.Uid uid, int maxInviteeCount,
            ...
            byte[] pAppParameter)
        {
            return StartFriendInvitationV2(uid, maxInviteeCount, description,
                pAppParameter, pAppParameter.LongLength);
        }

        [DllImport(Nn.DllName,
            CallingConvention = CallingConvention.Cdecl,
            EntryPoint = "nn_friends_StartFriendInvitationV2")]
        private static extern nn.Result StartFriendInvitationV2(nn.account.Uid uid, int maxInviteeCount,
            nn.friends.FriendInvitationGameModeDescriptionV2 description,
            [In] byte[] pAppParameter, long appParameterSize);
        ...
        public static nn.Result StartSendingFriendInvitation(nn.account.Uid uid,
            ...
            byte[] pAppParameter)
        {
            return StartSendingFriendInvitationV2(uid, pInvitees, pInvitees.Length, description,
                pAppParameter, pAppParameter.LongLength);
        }

        [DllImport(Nn.DllName,
            CallingConvention = CallingConvention.Cdecl,
            EntryPoint = "nn_friends_StartSendingFriendInvitationV2")]
        private static extern nn.Result StartSendingFriendInvitationV2(nn.account.Uid uid,
            [In] nn.account.NetworkServiceAccountId[] pInvitees, int inviteeCount,
            nn.friends.FriendInvitationGameModeDescriptionV2 description,
            [In] byte[] pAppParameter, long appParameterSize);
#endif
